...
EOT;
    ...
    {
        return $this->getSqlUsers($uid);
    }

    private function getSqlUsers($uid = '')
    {
        $db = $this->getInstance();
...
EOT;
    ...
    public function getAllNodes($uid)
    {
        ...
        $nodes = array_filter(array_merge((array) $Role, (array) $User));
    ...
    public function getUserMenu($uid)
    {
        ...
        $nodes = $this->getAllNodes($uid);
        $node = $this->getTableName('node');
        $sql = 'select * from ' . $node . ' where status=1 and menu=1 and id in(' . implode(',', $nodes) . ') order by sort desc';
        $list = $db->query($sql)->fetchAll($this->result_type);
        if (empty($list)) {
            return $list;
        }
    ...
    /**
     * desc : 所有节点，并选中用户拥有的节点
     * author : luoqi
     * time : 2017/03/21 14:42
     * param $uid int
     * @return array
     */
    public function AllToMe($uid, $self = false)
    {
        $db = $this->getInstance();
        $nodes = $this->getAllNodes($uid);
        $node = $this->getTableName('node');
    ...
    public function AllToRoles($role_id, $self = false)
    {
        ...
        $node = $this->getTableName('node');
        $sql = 'select id,name,sort,title,pid,icon,menu from ' . $node;
        $list = $db->query($sql)->fetchAll($this->result_type);
        if (empty($list)) {
            return $list;
        }
    ...
    public function addNode($params)
    {
        ...
        $sql = <<<EOT
            insert into {$node_table}
                  (`name`, `title`, `status`, `sort`, `pid`, `icon`, `menu`,`create_time`)
                  VALUES (  '{$params['name']}',
                            '{$params['title']}',
                            '{$params['status']}',
                            '{$params['sort']}',
...
EOT;
    ...
    {
        $db = $this->getInstance();
        $able_fields = ['name', 'title', 'status', 'sort', 'pid', 'icon', 'menu'];
        $condition_str = $this->getUpdateStr($params, $able_fields);
        $node_table = $this->getTableName('node');
        $sql = <<<EOT
